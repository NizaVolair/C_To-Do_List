 ...
 ** Author: Niza Volair
 ** Date: 08-02-15
 ** Description: Implementation file for a heap-based priority queue built with a dynamic array  -
...
int _smallerIndexHeap(DynArr *heap, int i, int j)
{
    /* Assert that both indexes are within bounds */
...
{
  /* Assert Heap is not empty then return the first element (the one at 0) */
...
void _adjustHeap(DynArr *heap, int max, int pos)
{
   /* Assert max index is within bounds and get indexes of children */
    assert(max <= sizeDynArr(heap));
    int leftidx = pos *2 + 1;
    int rightidx = pos *2 + 2;

    /* If right is smaller than max, find smallest child, if parent is larger swap */
    if(rightidx < max){
    ...
    /* If there is no right child, just compare with the left */
    else if(leftidx < max ){
...
{
   /* Assert th eheap isn't NULL and find the last element */
    assert(sizeDynArr(heap) > 0);
    int last = sizeDynArr(heap) - 1;
    if (last != 0)     /* make sure we have at least one element */
    ...
    if (last != 0)      /* Rebuild heap if more there was more than 1 element*/
...
{
    /* Assert heap is not empty and find max node*/
    assert(sizeDynArr(heap) > 0);

    int temp = sizeDynArr(heap);

    /* By checking through the last half of the nodes we move up to check the ones above */
    int i;
    for(i = (temp / 2 - 1); i >= 0; i--)
    {
        _adjustHeap(heap, temp, i);
    }
...
{
    /* Assert heap is not NULL and build it with the function */
    assert(sizeDynArr(heap) > 0);
    _buildHeap(heap);

    /* Sort nodes in place by swapping and rebuilding each time */
    int i;
